...
const { ApiKeyStore, bootstrap } = require('./lib/apiKeys');
const { assertPublicHttpUrl } = require('./lib/ssrfGuard');
const { isValidSessionId, redactForLog } = require('./lib/security');
const { createAuthMiddleware, requireAdmin } = require('./middleware/auth');
...
// ============================================
app.set('trust proxy', 1); // Railway/derrière un reverse proxy: nécessaire pour un req.ip correct
...
if (generatedAdminKey) {
    ...
    console.log('   générée automatiquement. Notez-la, elle ne sera plus affichée :');
    console.log(`   ${generatedAdminKey}`);
    console.log('   Définissez ADMIN_API_KEY en variable d\'environnement pour la');
    console.log('   fixer de façon stable entre les redémarrages.');
    console.log('================================================================');
}
const authenticate = createAuthMiddleware(apiKeyStore);

// Limiteur grossier par IP, actif avant même l'authentification: ralentit le
// bruteforce de clés API et les abus non authentifiés (y compris /api/health).
const ipLimiter = rateLimit({
    windowMs: 5 * 60 * 1000,
    limit: parseInt(process.env.IP_RATE_LIMIT_MAX || '300', 10),
    standardHeaders: true,
    legacyHeaders: false,
    message: { ok: false, error: 'Trop de requêtes depuis cette adresse IP, réessayez plus tard.' },
});
app.use(ipLimiter);
...
// ============================================
// Déclaré avant app.use(authenticate) ci-dessous: Express traite ce handler
// et termine la requête sans jamais atteindre le middleware d'authentification.
// `sessions` est initialisé plus bas dans ce module, mais comme ce handler ne
// s'exécute qu'au moment d'une requête HTTP (donc après le chargement complet
// du module), la référence est déjà disponible — aucun souci de TDZ ici.
...
// À partir d'ici, TOUTE requête doit présenter une clé API valide.
...
const keyLimiter = rateLimit({
    ...
    message: { ok: false, error: 'Limite de requêtes atteinte pour cette clé API, réessayez plus tard.' },
});
app.use(keyLimiter);
...
const startLimiter = rateLimit({
    ...
    legacyHeaders: false,
    keyGenerator: (req) => req.apiKey?.id || ipKeyGenerator(req.ip),
    message: { ok: false, error: 'Trop de créations de session WhatsApp, réessayez plus tard.' },
});
...
// ============================================
// LIMITES ANTI-ÉPUISEMENT DE RESSOURCES
// ============================================
// Chaque session ouvre un Chrome headless complet (Puppeteer) : sans plafond,
// un appelant peut en créer un nombre illimité et épuiser CPU/RAM/disque.
const MAX_SESSIONS_TOTAL = parseInt(process.env.MAX_SESSIONS_TOTAL || '50', 10);
const MAX_SESSIONS_PER_KEY = parseInt(process.env.MAX_SESSIONS_PER_KEY || '5', 10);

function countSessionsOwnedBy(apiKeyId) {
    let count = 0;
    for (const session of sessions.values()) {
        if (session.ownerKeyId === apiKeyId) count++;
    }
    return count;
}

// ============================================
// ISOLATION MULTI-TENANT : une session appartient à la clé API qui l'a créée.
// Une clé "standard" ne peut agir que sur ses propres sessions ; une clé
// "admin" peut agir sur toutes les sessions.
// ============================================
function resolveOwnedSession(req, res, sessionId) {
    if (!isValidSessionId(sessionId)) {
        res.status(400).json({ ok: false, error: "sessionId invalide : uniquement lettres, chiffres, '-' et '_', 64 caractères max" });
        return null;
    }
...
// Applique automatiquement, pour TOUTE route contenant :sessionId, la
// validation de format ET le contrôle d'appartenance (multi-tenant) —
// avant même d'entrer dans le handler de la route.
...
// ============================================
// ADMINISTRATION DES CLÉS API (réservé aux clés de portée "admin")
// ============================================
app.post('/api/admin/keys', requireAdmin, (req, res) => {
    const { name, scope } = req.body || {};
    try {
        const created = apiKeyStore.createKey({ name, scope: scope || 'standard' });
        res.status(201).json({
            ok: true,
            message: "Clé créée. Notez-la maintenant : elle ne sera plus jamais affichée en clair.",
            ...created,
        });
    } catch (error) {
...
app.get('/api/admin/keys', requireAdmin, (req, res) => {
    res.json({ ok: true, keys: apiKeyStore.list() });
});
...
app.post('/api/sessions/:sessionId/start', startLimiter, async (req, res) => {
    ...
    try {
        if (sessions.has(sessionId)) {
            const session = sessions.get(sessionId);
            // app.param('sessionId', ...) a déjà vérifié l'appartenance ci-dessus.
            return res.json({
                ok: true,
...
app.post('/api/sessions/:sessionId/webhook', async (req, res) => {
    ...
    if (!sessions.has(sessionId)) return res.status(404).json({ ok: false, error: 'Session non trouvée' });
    try {
        await assertPublicHttpUrl(url);
    } catch (error) {
        return res.status(error.statusCode || 400).json({ ok: false, error: `URL de webhook refusée: ${error.message}` });
    }
...
app.post('/api/messages/send-video', async (req, res) => {
    ...
    const session = resolveOwnedSession(req, res, sessionId);
    if (!session) return;
    if (session.status !== 'WORKING') return res.status(400).json({ ok: false, error: 'Session not working' });
...
app.post('/api/sessions/close-all', async (req, res) => {
    ...
    console.log(`🧹 Fermeture ${isAdmin ? 'de toutes les sessions' : 'des sessions de cette clé'} (${sessions.size} actives au total)...`);
...
app.get('/api/sessions', (req, res) => {
    ...
    sessions.forEach((session, sessionId) => {
        // Isolation multi-tenant: une clé "standard" ne voit que ses propres sessions.
        if (!isAdmin && session.ownerKeyId && session.ownerKeyId !== req.apiKey.id) return;
        sessionList.push({
            sessionId,
            status: session.status,
            phoneNumber: session.phoneNumber,
...
app.get('/api/messages/:messageId/status', (req, res) => {
    ...
    const owningSession = sessions.get(msg.sessionId);
    if (req.apiKey.scope !== 'admin' && owningSession?.ownerKeyId && owningSession.ownerKeyId !== req.apiKey.id) {
        return res.status(403).json({ ok: false, error: "Ce message appartient à une autre clé API." });
    }
    res.json({
        ok: true,
